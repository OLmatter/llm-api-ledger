...
### 铁律 17：禁用吊牌价，只记定价 + 优惠两层

**用户定义的核心概念（2026-07-23）**：

| 层级 | 定义 | 字段 | 例子 |
|---|---|---|---|
| ❌ **吊牌价/原价** | 厂商标的高昂"原价"，**没人在乎，不准记录** | 禁止用 `base_*` | Z.AI 单月付 \$72（实际月订阅定价 \$64.8） |
...
**核心规则**：
1. **禁止 `base_*` 字段**（吊牌价的载体）——lint 强制检查，yml 里出现 `base_*` 直接报错
...
4. **前端不准展示"原价"划线**——用户不在乎所谓原价，展示定价 + 优惠就够
