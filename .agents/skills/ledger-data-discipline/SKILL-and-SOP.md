...
## 1.11 性价比曲线模块（/value 页，2026-09-09 落地）

- 数据：build-plans.mjs 从榜单表格直接聚合（月度用量 ÷ 包月原价），**不看 API 按量价目**
- 双口径：智谱/Z.AI v3 行带 conservative_tokens_per_cny（全高峰下限=全非高峰上限÷2），
  页面可切换「标准用量 / 极限用量」两种排名；v2 实测行两模式同值
- 虚数行（ChatGPT/Claude Code Pro 等效美元拆分）打 virtual 标记，不参与曲线，
  在「等效折算参考」区单独归档
- 图形态：横向柱状（名称左列，根治标签重叠）；log/线性轴切换（数值悬殊 54:1）
- 厂商筛选 chips 只显示当前组实际存在的厂商；颜色深浅双值（useData isDark 响应式）
- 常驻口径说明随模式联动（极限/标准各自的完整定义），不许只放 hover title
...
- ❌ 文件只给相对路径或文件名，不给绝对地址
- ❌ 不给来源链接/预览链接
- ❌ **给文件清单 / git diff --stat 当汇报**（2026-08-31 用户原话「我不看文件，只看表格，以后记好了」）
  —— 用户不看文件列表，只看数据表格。汇报必须以 data-diff 表格为主体，
  「哪个套餐 / 哪个字段 / 之前 → 现在」一行一行列清楚；代码改动也用「行为对照表」表达，不罗列文件。
...
### 铁律 30：任何用量数字必须自带模型标签，禁止靠位置推断归属（2026-08-31 用户教学）

**用户原话**：「任何用量都得加模型比如 ZCode×1.5 310M。不能说它在 5.3 下面就是 5.3。另外 zai 也是智谱的」

**规则**：
1. 榜单上每一个用量数字（实测/宣称/折扣后/ZCode×1.5/邀请码后/DS 等价等）**必须自带 @模型标签**，
   例：`ZCode×1.5 310M @glm-5.3`。
2. **禁止靠渲染位置推断归属**——「它在 @glm-5.3 下面所以就是 5.3 的」不算标注。
   行序、缩进、分组在排序/换行/增删行后都会变，位置不是数据。
3. 数据层保证：每个用量字段在 build 产物（plans.json）里必须能追溯到唯一 model_id；
   渲染层保证：标签跟数字在同一个 DOM 节点里。
4. **zai = 智谱海外版**，与国内智谱同模型池、同 ZCode 权益、同规则。
   凡适用于智谱的铁律（模型标签、ZCode 折算、5.3-only 模型口径等）同等地适用于 zai。

**事故**（2026-08-31）：ZCode×1.5 子行只显示 `ZCode×1.5 310M`，没带 @模型标签，
用户指出「不能说它在 5.3 下面就是 5.3」——如果模型行排序变化或插入新模型，
子行数字会被安到错误模型头上，且无任何痕迹。

**自检**：改完任何用量展示后，遮住上下文只看单行——
「这个数字是哪个模型的？」答不上来 = 违反本条。
