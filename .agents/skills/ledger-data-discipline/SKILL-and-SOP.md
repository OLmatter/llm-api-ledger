...
## 0.6 数据采集三原则（按字段分工，不混）

**为什么单独列**：豆包 SOP 第 2-3 步混在一起，违反了本项目「三类价格不可混」（铁律 1）+「不准编官方数据」（铁律 16）。本节明确分工。

### 原则 A：定价类字段（pricing.*）—— 严格官方页

| 字段 | 来源 | 校验 |
|---|---|---|
| `original_monthly` / `original_yearly` | 厂商定价页直接看到的数字 | 必须能在官方页找到，不准凭印象/历史 |
| `currency` | 厂商定价页（CNY / USD） | 不准改 |

**禁止**：把邀请码价格/活动价/社群优惠价填进 original_*（违反铁律 1 + 17）。

### 原则 B：优惠类字段（pricing.intro_* / affiliate）—— 限时/特定人群

| 字段 | 来源 | 校验 |
|---|---|---|
| `intro_monthly` / `intro_quarterly` / `intro_yearly` | 厂商「首单优惠」/「限时活动」页 | 必须有 expires（铁律 11） |
| `affiliate.code` / `affiliate.discount` / `affiliate.url` | 邀请码分享页（铁律 7 6 字段） | 必须有 expires |

**禁止**：把渠道返点/社群折扣当官方原价（违反铁律 17「只记定价+优惠两层」）。

### 原则 C：实测类字段（measurements + tokens_measured）—— 用户实测

| 字段 | 来源 | 校验 |
|---|---|---|
| `tokens_measured` | 用户实测 / 多源聚合（median） | 单源必须标 disputed（铁律 10） |
| `requests_official` | 厂商定价页明确写出的请求次数 | 不准脑补（铁律 16） |
| `tokens_official_claimed` | 厂商公布的 token 总量（仅 MiniMax/Kimi/Anthropic） | 必须能在官方页找到 |

**禁止**：把「凭印象」「凭 1% 用量反推」当实测（违反 feedback_only_fill_what_user_provided）。

---

## 0.7 厂商添加流程（用户授权，不自动巡检）

**用户原话**（2026-07-31 第 7 条回应）：「每次我会让你加。比如去加腾讯的。」
...
用户：「加腾讯」
   ↓
我读 SOP + NDNS + 搜 memory（启动协议 0.5）
   ↓
我搜腾讯 API 当前在售套餐 + 价格 + 模型（WebSearch 多源印证）
   ↓
我展示给用户：「腾讯现在有 X/Y/Z 3 个套餐，要全加吗？要加哪些模型？」
   ↓
用户确认 / 调整范围
   ↓
我建 vendor.yml + plan.yml + build-plans.mjs 改动
   ↓
...
**豆包 SOP 第 1 步「加载固定常驻厂商列表 + 叠加本轮新增 + 剔除死平台」违反本原则**——这等于 agent 自主判断厂商去留。本节明确：厂商去留由用户决定。

---

## 0.8 豆包 SOP 评审记录（2026-07-31）

用户让 Claude 评审豆包 SOP「API 套餐榜单 完整精细化 SOP（最终可落地版）」，9 步流程。评审结论：**部分采纳，部分拒绝**。

### 采纳的部分（已纳入 SOP）

| 豆包条款 | 采纳位置 | 备注 |
|---|---|---|
| 第 3 步「价格+优惠+官方宣称参数」采集 | 0.6 原则 A/B/C | 重写为三层分工，不混 |
| 第 4 步「业务合理性校验」 | 0.7 + 铁律 18 | 重写为「用户授权」原则，禁止 agent 自主判断瓶颈 |
| 第 6 步「对照历史表格生成初稿」 | 启动协议 0.5 第 4 步 | 历史表格 = 当前 yml，不是历史截图 |

### 拒绝的部分（违反本项目 18 铁律）

| 豆包条款 | 拒绝原因 | 对应铁律 |
|---|---|---|
| 第 4 步第 1 条「月 44k...判定不适合 Agent」 | 主观评估,榜单只列数据不评估 | 铁律 18 + 用户原话「不要评估」 |
| 第 4 步第 3 条「判断真实瓶颈」 | agent 没资格判断瓶颈,这是用户决策 | 铁律 18 |
| 第 4 步第 4 条「校验优惠真实性」 | 优惠真实性由用户确认,不是 agent 校验 | 铁律 18 |
| 第 1 步「加载常驻厂商列表 + 自动剔除死平台」 | 厂商去留由用户决定 | 铁律 18 + 用户原话「每次让你加」 |
| 「除非用户同意才可以进行反推」 | 反推需要**预先**用户授权,不是事后同意 | 用户裁定（启动协议 0.5 反推流程） |
| 第 7 步「替换正式源文件」无前置校验 | commit 前必须有 git diff 自检 + 用户确认 | 铁律 18 |
| 「沉淀到本地记忆」 | 必须写 NDNS,不准本地替代 | 设备级 CLAUDE.md §7 |

### 豆包 SOP 未涉及但本项目必需

- **M1 写回纪律**（5 个必须写时机）—— 设备级 CLAUDE.md §6 强制
- **铁律 18 零修改原则**—— 本项目独创,不在豆包 SOP
- **反推授权原则**—— 用户 2026-07-31 裁定,不在豆包 SOP
- **build 函数不擅自禁用**—— 本项目经验,不在豆包 SOP

**结论**：豆包 SOP 不能直接落地,经过本项目 18 铁律过滤后,只采纳**采集纪律**部分,**自主决策类条款全部拒绝**。本文件 18 铁律 + 0.5-0.8 节是唯一权威 SOP。
