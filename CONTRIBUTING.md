...
1. Fork 仓库，把你的截图或导出文件放进 `data/<vendor_id>/scraped/`。
   ...
   - 不知道 `vendor_id` 就先开 Issue 问，或按方式 2 新建厂商路径。
...
5. 提 PR，标题写清 `[evidence] zhipu GLM Coding Plan v3 overview 2026-07-31`。
...
适用：你发现了榜单没收录的厂商，或新版本套餐（例：新出的 GLM Coding Plan v4、Anthropic Max-20x 之外的 SKU）。
...
1. 下载并跑探针（见 README）。连续跑 ≥ 7 天。
2. 探针导出页（`/__ledger__/export`）会按 `.github/PULL_REQUEST_TEMPLATE.md` 自动生成脱敏 Markdown。
...
- 修改 `docs/index.md` 里已经在用的"最新更新"时间（commit 时由 build 自动维护）
